...
class iPanorama_App {
	...
	function run() {
		...
		define('IPANORAMA_PLUGIN_UPLOAD_URL', $upload_dir['baseurl'] . '/' . IPANORAMA_PLUGIN_NAME . '/');
	...
	function shortcode($atts) {
		extract(shortcode_atts(array('id'=>0, 'class'=>NULL, 'sceneid'=>NULL, 'width'=>NULL,'height'=>NULL), $atts));
		
		if(!$id) {
			return '<p>' . esc_html__('Error: invalid ipanorama shortcode attributes', IPANORAMA_PLUGIN_NAME) . '</p>';
		}
		...
		if($item) {
			...
			echo '<div ';
			echo 'class="ipanorama ' . 'ipnrm-' . $id . ($class ? ' ' . $class : '') . '"'; // $class from shortcode
			...
			//=============================================
			// STORE BEGIN
			echo '<div class="ipnrm-store">' . PHP_EOL;
			...
			foreach($itemData->scenes as $sceneKey => $scene) {
				...
				foreach($scene->markers as $markerKey => $marker) {
					...
					echo '<div class="ipnrm-data" data-marker-id="' . $marker->id . '">' . PHP_EOL;
					echo do_shortcode($marker->popover->data);
					echo '</div>' . PHP_EOL;
		...
		return '<p>' . esc_html__('Shortcode ipano', IPANORAMA_PLUGIN_NAME) . '</p>';
	}
	...
	*/
	function do_parse_request($result) {
		...
		if(preg_match('/ipanorama\/virtualtour\/([a-z0-9_-]+)/', $current, $matches)) {
			...
			if(is_numeric($matches[1])) {
				...
				if($virtualtour_id != null) {
					...
					$query = $wpdb->prepare('SELECT * FROM ' . $table . ' WHERE id=%d', $virtualtour_id);
					$item = $wpdb->get_row($query, OBJECT);
					if($item) {
						if(!$item->active) {
							return;
				...
				if($virtualtour_slug != null) {
					...
					$query = $wpdb->prepare('SELECT * FROM ' . $table . ' WHERE slug=%s', $virtualtour_slug);
					$item = $wpdb->get_row($query, OBJECT);
					if($item) {
						if(!$item->active) {
							return;
	...
	function admin_menu_page_items() {
		...
		if($page===IPANORAMA_PLUGIN_NAME) {
			...
			$globals = array(
				...
				'ajax_msg_error' => esc_html__('Uncaught Error', IPANORAMA_PLUGIN_NAME) //Look at the console (F12 or Ctrl+Shift+I, Console tab) for more information
			);
			...
			if($action && $nonce && wp_verify_nonce($nonce, IPANORAMA_PLUGIN_NAME)) {
				global $wpdb;
				$table = $wpdb->prefix . IPANORAMA_PLUGIN_NAME;
				...
				if($action=='duplicate') {
					...
					if($item && (current_user_can('manage_options') || get_current_user_id()==$item->author) ) {
						...
						$itemData->title = esc_html__('[Duplicate] ', IPANORAMA_PLUGIN_NAME) . $itemData->title;
			...
			if($flag) {
				if($inputId) {
					...
					if($result) {
						...
						$data['msg'] = esc_html__('Item created', IPANORAMA_PLUGIN_NAME);
					} else {
						$error = true;
			...
			if(!$error) {
				if(wp_is_writable(IPANORAMA_PLUGIN_UPLOAD_DIR)) {
					...
					if($wp_filesystem) {
						...
						// delete unused files
						$files = glob($this->joinPaths($dir_preload,'*.json'));
						foreach($files as $file) {
							...
							foreach($itemData->scenes as $key => $scene) {
								if(strcasecmp($scene->guid, $fileName) == 0) {
							...
							if($doAction) {
								$filesToDelete = glob($this->joinPaths($dir_preload, $fileName . '.*'));
						...
						foreach($itemData->scenes as $key => $scene) {
							if($scene->type != 'cubesix' && $scene->type != 'gsv') {
								if($scene->preload->active && $scene->images->main->url) {
									$imageInfo = pathinfo($scene->images->main->url);
									$imageExt = $imageInfo['extension'];
									
									$imageUrlSrc = $scene->images->main->url;
									$imageUrlDst = $this->joinUrls($url_preload, $scene->guid . '.' . $imageExt);
									
									$jsonFile = $this->joinPaths($dir_preload, $scene->guid . '.' . 'json');
									$jsonData = array(
										'id' => $scene->id,
										'title' => $scene->title->data,
										'image' => $imageUrlSrc
									);
									
									if(file_exists($jsonFile)) {
										$fileData = $wp_filesystem->get_contents($jsonFile);
										$fileJsonData = json_decode($fileData);
										
										if(strcasecmp($fileJsonData->image, $imageUrlSrc) == 0) {
											$wp_filesystem->put_contents($jsonFile, json_encode($jsonData), FS_CHMOD_FILE); // refresh data
											
											// update json config
											for($i=0, $size=count($itemConfig->scenes); $i<$size; ++$i) {
												if(strcasecmp($itemConfig->scenes[$i]->guid, $scene->guid) == 0) {
													$itemConfig->scenes[$i]->imagePreload = $imageUrlDst;
												}
											}
									...
									// genearate the preload image
									$imageMaxWidth  = 512;
									$imageMaxHeight = 512;
									$imageTmp = download_url($imageUrlSrc);
									
									$error = true;
									if(!is_wp_error($imageTmp)) {
										$imageEditor = wp_get_image_editor($imageTmp);
										if(!is_wp_error($imageEditor)) {
											if(!is_wp_error($imageEditor->resize($imageMaxWidth, $imageMaxHeight))) {
												$imageResized = $imageEditor->save();
												
												if(!is_wp_error($imageResized)) {
													$imageSrc = $imageResized['path'];
													$imageDst = $this->joinPaths($dir_preload, $scene->guid . '.' . $imageExt);
													
													copy($imageSrc, $imageDst);
													
													$wp_filesystem->put_contents($jsonFile, json_encode($jsonData), FS_CHMOD_FILE);
									...
									if($error) {
										$data['msg'] = esc_html__('Server error. Can\'t create preload images.', IPANORAMA_PLUGIN_NAME);
										break;
									}
								}
							}
