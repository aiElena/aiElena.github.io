jQuery(document).ready(function() {
    ...
    targetToggle.on('change ifChanged', function(e) {
        ...
        jQuery(self.data('target-toggle')).each(function() {
            var $this = jQuery(this),
                showValue = $this.data('target-show'),
                hideValue = $this.data('target-hide');
    ...
    // $(".wcuSwitcherRadioLabel").click(function(){
    //     var name = $(this).closest("input:radio").attr("name");
    //     var currVal = $(this).closest("input:radio").val();
    //     var oldVal;
    //     $("[name='"+name+"']").each(function(index){
    //         var attr = $("[name='"+name+"']:eq("+index+")").attr('checked');
    //         if (typeof attr !== typeof undefined && attr !== false) {
    //             oldVal =$("[name='"+name+"']:eq("+index+")").val();
    //         }
    //         $("[name='"+name+"']:eq("+index+")").parent().removeClass('checked');
    //         $("[name='"+name+"']:eq("+index+")").removeAttr('checked');
    //     });
    //     $(this).closest("input:radio").attr("checked",true);
    //     $(this).addClass("checked");
    //     var currentTab = $(this).closest(".wcuTabContent");
    //     $(currentTab).find("tr").each(function(index){
    //
    //         if ( $(this).attr('data-target-hide') === oldVal ) {
    //             $(this).show();
    //         }
    //         if ( $(this).attr('data-target-hide') === currVal ) {
    //             $(this).hide();
    //         }
    //         if ( $(this).attr('data-target-show') === oldVal ) {
    //             $(this).hide();
    //         }
    //         if ( $(this).attr('data-target-show') === currVal ) {
    //             $(this).show();
    //         }
    //
    //     });
    ...
    if (jQuery("select[name='wcu_options[options][converter_type]']").val() === 'free_converter') {
        jQuery(".wcuOptionsFreeConverterApiKey").fadeIn();
    } else {
        jQuery(".wcuOptionsFreeConverterApiKey").fadeOut();
    }
    ...
    jQuery('.wcuRateCustom').on('input', function(e) {
...
function wcuLookPreviewAjax(moduleName) {
    ...
    /*var data = jQuery("#mainform input[name*='["+divClass+"]'], #mainform select[name*='["+divClass+"]']").serializeArray();
    var pl =  jQuery("#mainform input[name='pl']").val();

    data.push({
        name : 'mod',
        value : jQuery("#mainform input[name='mod']").val(),
    });
    data.push({
        name : 'action',
        value : jQuery("#mainform input[name='action']").val(),
    });
    data.push({
        name : 'pl',
        value : jQuery("#mainform input[name='pl']").val(),
    });*/
    ...
    /*jQuery.ajax({
        type: "POST",
        dataType: 'html',
        url: ajaxurl+"?mod="+divClass+"&action="+moduleName+"",
        data: data,
        success: function(data) {
            data = data.substring(0, data.length - 1);
            jQuery(".wcuTabContentPreviewInner"+divClass+"").html(data);
        }
    });*/
};
